  ...
  return (
    <div className={sidebarClassNames}>
        ...
        <div className={`flex gap-3 justify-between md:justify-normal items-center pt-8 ${isSidebarCollapsed ? "px-5" : "px-8"}`}>
            <Image src="https://s3-inventory-management-andy.s3.ap-southeast-2.amazonaws.com/logo.png"
                         alt="AndyStock-Logo"
                          width={27}
                          height={27}
                          className="rounded w-8"/>
            <h1 className={`${isSidebarCollapsed ? "hidden" : "block"} font-extrabold text-2xl`}>ANDYSTOCK</h1>
            <button
        ...
        <div className={` ${isSidebarCollapsed ? "hidden" : "block"} mb-10`}>
            <p className='text-center text-xs text-gray-500'>&copy; {year} AndyStock</p>
        </div>
    </div>
  )
}
